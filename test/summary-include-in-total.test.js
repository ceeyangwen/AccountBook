...
test('隐藏账号仍应按 includeInTotal 汇总，记录名称默认脱敏', () => {
  ...
  const hiddenRecord = indexPage.data.groupedRecords[0].records.find(record => record.id === 'hidden-expense');
  assertEqual(indexPage.data.monthlyExpense, '50.00', '首页支出应包含隐藏且计入总资产账号');
  assertEqual(indexPage.data.monthlyIncome, '100.00', '首页收入应保持只统计计入账号');
  assertEqual(hiddenRecord.accountDisplayName, '已隐藏账号', '隐藏账号记录名称默认脱敏');

  const statisticsPage = loadPage('../miniprogram/pages/statistics/statistics.js', appData);
  statisticsPage.data.period = 'month';
  statisticsPage.processStatistics();

  assertEqual(statisticsPage.data.totalExpense, '50.00', '统计页支出应包含隐藏且计入总资产账号');
  assertEqual(statisticsPage.data.totalIncome, '100.00', '统计页收入应保持只统计计入账号');
});
